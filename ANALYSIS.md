...
**技术栈**：React 18 / Vite 7 / Tailwind CSS 4 / IndexedDB / Service Worker / Cloudflare Workers KV / DOMPurify / marked
...
2. **跨设备同步**
   - Cloudflare Workers KV 后端，UUID-based Sync ID
   - UNION merge 策略（幂等、无冲突），readStatus 按首次已读时间合并，readingList 按墓碑优先合并
   - 自动 push（debounce 3s）+ 手动同步 + 初始同步
   - readingList 上传时剥离 content 避免 KV payload 超限，下载后从本地 articles store 回填
...
- **Sync ID 明文传输在 URL query**：`/sync?key=<id>`，GET 请求的 query 会出现在 CF Worker 日志、浏览器历史、Referer header 中。Sync ID 等同密钥，应改用 POST body 或 Authorization header
...
2. **同步设计精巧**：UNION merge 是正确的选择——readStatus 和 readingList 都是单调增集合，取并集天然幂等。墓碑机制解决了"删除也要传播"的经典分布式问题。stripReadingListForWire 剥离 content 避免 KV 超限，再从本地回填，细节到位。
